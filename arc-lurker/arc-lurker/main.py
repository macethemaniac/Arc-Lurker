try:
    # Initialize Discord and Twitter clients
    if not all([X_API_KEY, X_API_SECRET, X_ACCESS_TOKEN, X_ACCESS_SECRET, X_BEARER_TOKEN, ETHERSCAN_API_KEY]):
        raise ValueError("Missing required API credentials")
    
    # Test Twitter API connection
    client.get_me()
    
    print("Successfully initialized API connections")
...
def validate_eth_contract(address):
    ...
    try:
        ...
        if not address.startswith('0x') or len(address) != 42:
            logger.error(f"Invalid ETH address format: {address}")
            return None
            
        if 'pump' in address:
            logger.warning(f"Skipping potential pump token: {address}")
            return None
        
        try:
            # No indented code here!
            except Exception as e:
...
def validate_sol_contract(address):
    ...
    try:
        ...
        if len(address) < 32 or len(address) > 44:
            logger.error(f"Invalid SOL address length: {address}")
            return None
            
        valid_chars = set('123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz')
        if not all(c in valid_chars for c in address):
            logger.error(f"Invalid SOL address characters: {address}")
            return None
        ...
        logger.error(f"Error in basic SOL validation for {address}: {str(e)}")
        return None
    ...
    try:
        ...
        if response.status_code == 404:
            logger.warning(f"SOL contract not found: {address}")
            return None
            
        if response.status_code != 200:
            logger.error(f"SOL API error {response.status_code} for {address}")
            return None
            
        data = response.json()
        if not data:
            logger.error(f"Empty response for SOL contract: {address}")
            return None
        ...
        logger.warning(f"Invalid token data for SOL contract: {address}")
        return None
        
    except (requests.exceptions.RequestException, ValueError) as e:
        logger.error(f"Error validating SOL contract {address}: {str(e)}")
        return None
...
RATE_LIMIT_DELAY = 300  # 5 minutes base delay
current_delay = RATE_LIMIT_DELAY
MAX_DELAY = 900  # 15 minutes maximum delay
...
@tasks.loop(seconds=300)  # Increased to 5 minutes to avoid rate limits
async def monitor_x():
    try:
        ...
        if '429' in str(e):
            logger.warning(f"Rate limit hit, pausing monitoring for {current_delay} seconds")
            monitor_x.change_interval(seconds=current_delay)
            await handle_rate_limit()
        else:
            logger.error(f"Twitter API error: {str(e)}")
    except Exception as e:
        ...
        try:
            ...
            if username not in last_posts or last_posts[username][
                ...
                if view_count >= VIEW_SURGE_THRESHOLD:
                    await channel.send(
                        f"🚨 [@{username}](https://twitter.com/{username}/status/{tweet_id}) View Surge: {view_count} views on '{tweet_text[:50]}...'"
                    )
        ...
        try:
            ...
            if influencer_tweets.data:
                ...
                for tweet in influencer_tweets.data:
                    author = await safe_twitter_request(client.get_user, id=tweet.author_id)
                    author_username = author.data.username if author and author.data else tweet.author_id
                    influencer_alert += f"\n- [@{author_username}](https://twitter.com/{author_username}/status/{tweet.id}): '{tweet.text[:50]}...'"
